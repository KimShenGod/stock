    ...
    if df_filename not in local_zipfile_list:
        ...
        try:
            ...
            if os.path.exists(local_zipfile_path):
                os.remove(local_zipfile_path)  # 删除无效的zip文件
            print(f'文件{df_filename}下载损坏或不是有效的zip文件，已删除，跳过此文件')
        except FileNotFoundError:
            print(f'文件{df_filename}未找到，可能下载失败，跳过此文件')
        except Exception as e:
            if os.path.exists(local_zipfile_path):
                os.remove(local_zipfile_path)  # 删除可能损坏的zip文件
            print(f'处理文件{df_filename}时发生错误: {e}，已删除文件，跳过此文件')
...
for zipfile_filename in local_zipfile_list:
    ...
    # 从文件名中提取年份信息，格式为gpcwYYYYMMDD.zip
    try:
        file_year = int(zipfile_filename[4:8])
    except ValueError:
        file_year = current_year  # 如果无法提取年份，默认按当前年份处理
    
    # 对于5年前的旧文件，跳过更新检查，因为财务数据不会再变化
    if file_year < current_year - 5:
        print(f'{zipfile_filename} 是 {file_year} 年的文件，跳过更新检查')
        continue
    ...
    if file_md5 not in tdx_txt_df['md5'].tolist():  # 本机zip文件的md5与服务器端不一致
        ...
        try:
            ...
            if os.path.exists(local_zipfile_path):
                os.remove(local_zipfile_path)  # 删除无效的zip文件
            print(f'文件{zipfile_filename}下载损坏或不是有效的zip文件，已删除，跳过此文件')
        except FileNotFoundError:
            print(f'文件{zipfile_filename}未找到，可能下载失败，跳过此文件')
        except Exception as e:
            if os.path.exists(local_zipfile_path):
                os.remove(local_zipfile_path)  # 删除可能损坏的zip文件
            print(f'处理文件{zipfile_filename}时发生错误: {e}，已删除文件，跳过此文件')
...
for i in range(df_gbbq.shape[0]):
    category_code = str(df_gbbq.iat[i, df_gbbq.columns.get_loc("类别")])
    if category_code in category:
        df_gbbq.iat[i, df_gbbq.columns.get_loc("类别")] = category[category_code]
    else:
        df_gbbq.iat[i, df_gbbq.columns.get_loc("类别")] = f'未知类别_{category_code}'
        print(f'发现新的股本变迁类别: {category_code}，已添加为未知类别')
